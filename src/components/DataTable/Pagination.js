...
export default function ({ currentPage, setPage, lastPage, style }) {
	...
	return (
		<>
			...
			<div className="border-end ps-0">
				{pages && (pages.length > 1) ? (`${t('General|of')} ${(lastPage == 0) ? 1 : lastPage} ${t("General|Pages")}`) : (`${t('General|of')} ${(lastPage == 0) ? 1 : lastPage} ${t('General|Page')}`)}
			</div>
